...
class UserService {
    ...
    public createUser = async (
        ...
        try {
            const serializeObj: User = await this.model.create({name, email, password, role});
    ...
    public fetchUserById = async(userId: string): Promise<User | Error> => {
        try {
            const obj: User = await this.model.findById(userId as string).exec();
    ...
    public updateUser = async(userId: string, newObj: Partial<User>): Promise<User | Error> => {
        try {
            const updatedObj = await this.model.findByIdAndUpdate(userId as string, newObj as User,{new: true}).exec();
